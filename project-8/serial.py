...
#for clarity, sometimes is referenced to the report
...
Y = f(x) #array with the last approximation to the ODE
...
#iterates over the solutions found to get enhanced
#results, the best one beeing the last of all
@njit
def MCS(K,N,x,Y,h,q):
    for i in range(q):
        Y_C(K,N,x,Y,h)
