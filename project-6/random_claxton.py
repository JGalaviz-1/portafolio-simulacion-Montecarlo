...
w = 50 #number of CLaxton-Benson iterations
m = 5
K = .15
...
#this time use the positions at the end of the random walk
coord = np.loadtxt(f"TBU_{N}_{m}.txt") #laod the coordinates
t1 = time.time() #to compute the time taken
CB(coord) #perform the claxton-benson method
t2 = time.time()
...
with open(f"coord_R-C_{w}.txt", "w") as f:
    f.write(f"{N}\n")
