...
m = 5 #number of montecarlo cycles
...
#a montecarlo simulation wrapper
@njit
def MCS(crd,N,m):
    #perform m montecarlo cycles
    for j in range(m):
        for i in range(N):
            z = np.random.randint(0,N-1) #select a random point
            #move it randomly, so a trial position is generated
            a1 = crd[z,0] + 2*np.random.rand()-1
            a2 = crd[z,1] + 2*np.random.rand()-1
            a3 = crd[z,2] + 2*np.random.rand()-1
            ...
            if f_eng(a1,a2,a3,z,crd) < f_eng(crd[z,0],crd[z,1],crd[z,2],z,crd):
